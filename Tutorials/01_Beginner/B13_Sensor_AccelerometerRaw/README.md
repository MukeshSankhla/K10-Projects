...
This tutorial demonstrates how to read the onboard **SC7A20H 3-Axis Accelerometer** and present inertial telemetry using an **Aerospace Avionics HUD**.

You will learn how to sample X, Y, and Z gravitational force components in milli-gravities (mG), render individual axis telemetry meters with zero-centered deviation bars, and calculate total resultant G-force magnitude in real time.

### Expected Behavior
- **Avionics Header**: Gunmetal card with radar-green typography.
- **3-Axis Telemetry Cards**: Individual gauges for X (Pitch), Y (Roll), and Z (Normal/Gravity).
- **Zero-Centered Deviation Bars**: Visual indicators showing tilt direction and gravitational acceleration.
- **Total G-Force Metric**: Real-time vector magnitude calculation.
