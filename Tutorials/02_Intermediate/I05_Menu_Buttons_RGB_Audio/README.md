...
This tutorial demonstrates a complete **Multi-Peripheral System Menu** combining screen graphics, dual pushbuttons, onboard WS2812 RGB LED lighting effects, and buzzer tones on the UNIHIKER K10.

Features:
- **Interactive Menu Control**: Scroll through options with **Button [A]** and trigger hardware actions with **Button [B]**.
- **Action 1 (Red Alarm Strobe)**: Sets RGBs to full Red and sounds an 880 Hz alert beep.
- **Action 2 (Emerald Calm Mode)**: Sets RGBs to soft emerald green and sounds a mellow 523 Hz C5 chime.
- **Action 3 (Cyber Cyan Pulse)**: Emits vibrant cyan lighting and plays an ascending dual-tone chime (E5 $\to$ G5).
- **Action 4 (Melodic Fanfare)**: Displays a tricolor RGB rainbow pattern while playing a 4-note ascending musical fanfare.
- **Action 5 (Mute & OFF)**: Silences audio and turns off all RGB LEDs.

---

## How it works

1. **Menu Navigation Architecture**:
   - `activeIndex` tracks the targeted option.
   - When Button B triggers execution, `executeOption(index)` runs a `switch-case` dispatcher.
2. **Synchronized Hardware Actuation**:
   - `k10.rgb->write(index, color)` manipulates individual or collective WS2812 pixels.
   - `music.playTone(freq, duration)` synthesizes square-wave audio frequencies through the onboard buzzer.
   - The UI immediately updates its dynamic status badge to match the active mode.
3. **Dynamic Partial Component Refresh**:
   - Header banner and footer navigation controls are rendered once in `setup()`.
   - On scrolling with Button A, only the previous and newly targeted menu item cards are redrawn.
   - On triggering with Button B, only the mode status card is erased and updated, keeping visual presentation rock-solid without screen flicker.
...
| :--- | :--- | :--- | :--- |
| `k10.rgb->write(index, color)` | `int index, uint32_t color` | `void` | Writes color to specific or all (`-1`) LEDs. |
| `music.playTone(freq, duration)` | `uint16_t freq, uint32_t duration` | `void` | Generates a tone on the buzzer. |
| `music.stopPlayTone()` | None | `void` | Stops any currently active tone. |
| `k10.canvas->canvasRectangle(...)` | Geometry & colors | `void` | Draws menu cards and status banner. |
