...
3. **Dynamic Partial Component Refresh**:
   - The status bar shell, battery gauge, album art plinth & vinyl disc rings, and bottom controls guide are drawn once in `setup()` (`drawScreenChrome()`).
   - Toggling Play/Pause with Button A updates only the status bar glyph and vinyl center label (`updatePlayPauseStatus()`).
   - Changing tracks with Button B updates only the track title/artist/index metadata box (`updateTrackMetadata()`).
   - Timeline progression updates only the scrub bar line and timestamp text (`updateScrubTimeline()`), providing flicker-free retro iPod aesthetics.
